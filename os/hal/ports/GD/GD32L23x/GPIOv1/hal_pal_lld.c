...
/**
 * @file    hal_pal_lld.c
 * @brief   GD32 PAL subsystem low level driver source.
 *
 * @addtogroup PAL
 * @{
 */
                           ...
                           iomode_t mode)
{
  ...
  uint32_t ctl   = (mode & PAL_GD32_CTL_MASK) >> 0;
  uint32_t ot  = (mode & PAL_GD32_OMODE_MASK) >> 2;
  uint32_t ds = (mode & PAL_GD32_OSPD_MASK) >> 3;
  uint32_t pud   = (mode & PAL_GD32_PUD_MASK) >> 5;
  ...
  while (true) {
    if ((mask & 1) != 0) {
      ...
      m2 = 3 << (bit * 2);
      m4 = 15 << ((bit & 7) * 4);
      port->OMODE  = (port->OMODE & ~m1) | ot;
      port->OSPD = (port->OSPD & ~m1) | ds;
      port->PUD   = (port->PUD & ~m2) | pud;
      if ((mode & PAL_GD32_CTL_MASK) == PAL_GD32_CTL_ALTERNATE) {
        /* If going in alternate mode then the alternate number is set
           before switching mode in order to avoid glitches.*/
        if (bit < 8)
          port->AFSEL0 = (port->AFSEL0 & ~m4) | altrmask;
        else
          port->AFSEL1 = (port->AFSEL1 & ~m4) | altrmask;
        port->CTL   = (port->CTL & ~m2) | ctl;
      }
      else {
